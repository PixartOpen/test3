...
bool OTS_Sensor_Init(void)
{
	...
	if(sensor_pid == 0x31)
	{
		...
		WriteData(0x7F, 0x00); 	// switch to bank0, not allowed to perform OTS_RegWriteRead
		WriteData(0x06, 0x97); 	// software reset (i.e. set bit7 to 1). It will reset to 0 automatically
									// so perform OTS_RegWriteRead is not allowed.
		delay_ms(1); 				// delay 1ms
		
		OTS_WriteRead(0x09, 0x5A);// disable write protect
		OTS_WriteRead(0x0D, 0x65);// set X-axis resolution (depends on application)
		OTS_WriteRead(0x0E, 0xFF);// set Y-axis resolution (depends on application)
		OTS_WriteRead(0x19, 0x04);// set 12-bit X/Y data format (depends on application)
		OTS_WriteRead(0x4B, 0x04);// ONLY for VDD=VDDA=1.7~1.9V: for power saving
